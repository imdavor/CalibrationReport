...
def open_file():
    ...
    if file_path:
        ...
        filename_label.config(
            text=f"Datoteka: {os.path.basename(file_path)}"
        )  # Ažuriraj label s imenom datoteke
...
def create_report(file_path):
    ...
    try:
        with open(file_path, newline="", encoding="utf-16") as csvfile:
            ...
            for row in reader:
                ...
                actual_x = float(row["X/r"])
                actual_y = float(row["Y/a"])
                tree.insert("", tk.END, values=(f"Circle {row_count}", x_values[row_count], y_values[row_count], actual_x, actual_y, x_error, y_error))
                # tree.insert(
                #     "",
                #     tk.END,
                #     values=(
    ...
    for col in tree["columns"]:
        ...
        for row_id in tree.get_children():
            ...
            tree.column(
                col, width=max(tree.column(col)["width"], font.measure(str(cell_value)))
            )
    ...
    window_height = 150 + (
        displayed_rows * row_height
    )  # Ukupna visina prozora (150 je za gornje elemente i gumbiće)
    report_window.geometry(f"800x{window_height}")  # Postavi visinu prozora

    # Dodaj gumbe za Print i Zatvori
    print_button = tk.Button(
        report_window, text="Print", command=lambda: print_report(tree)
    )
    close_button = tk.Button(
        report_window, text="Zatvori", command=report_window.destroy
    )
    ...
    report_button.config(
        state=tk.DISABLED
    )  # Onemogući ponovno kreiranje izvještaja dok ne odabereš novu datoteku
...
def print_report(tree):
    ...
    for row_id in tree.get_children():
        ...
        for i, value in enumerate(row_values):
            max_widths[i] = max(
                max_widths[i], c.stringWidth(str(value), "Helvetica", 10)
            )
    ...
    for row_id in tree.get_children():  # Prođi kroz retke
        ...
        for i, value in enumerate(row_values):
            ...
            c.drawString(x_offset + sum(max_widths[:i]), y_offset, formatted_value)
    ...
    messagebox.showinfo(
        "PDF Spremljen", f"Izvještaj je spremljen kao {os.path.basename(pdf_file)}"
    )
...
report_button = tk.Button(
    ...
    text="Kreiraj report",
    state=tk.DISABLED,
    command=lambda: create_report(file_path),
)
